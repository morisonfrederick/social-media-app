...
function FriendRequest() {
  ...
  const friendRequsetHandler = async(friendId:string,option:string):Promise<void> =>{
        if(user&&user.id){
          try {
            const response =await apiClient.post(`/managefriends`,{userId:user.id,friendId,option})
  ...
  return (
    <div className="flex flex-col h-screen justify-center items-center bg-gray-100">
      ...
      <div className="flex-grow bg-white rounded-2xl w-96 m-4 shadow-lg p-6">
        <h2 className="text-2xl font-semibold text-gray-800 mb-4 text-center">Friend Requests</h2>
          ...
          friendrequests.map((friend, index) => (
            ...
            >
              ...
              <div className="flex space-x-2">
                <button onClick={()=>friendRequsetHandler(friend._id,'accept')} className="px-3 py-1 bg-green-500 text-white text-sm rounded hover:bg-green-600 transition-colors duration-200">
                  Accept
                </button>
                <button onClick={()=>friendRequsetHandler(friend._id,'reject')} className="px-3 py-1 bg-red-500 text-white text-sm rounded hover:bg-red-600 transition-colors duration-200">
                  Reject
                </button>
              </div>
            </div>
