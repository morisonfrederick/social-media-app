...
function Find() {
    ...
    const friendRequestHandler =async(friendId:string)=>{
      console.log('friend request send');
        try {
          const response = await apiClient.post(`/friendrequest`,{friendId,userString});
  ...
  return (
    ...
    <Navbar />
      <div className="flex-grow m-4 bg-slate-500 rounded-2xl shadow-2xl p-8 sm:w-96 w-11/12 max-w-lg h-[80%]">
        ...
        <div className="flex  w-full h-60 rounded-md mt-5 bg-gray-800">
          {users.length > 0 ? (
            <ul className="text-white p-4">
              {users.map((user: any, index: number) => (
                <li key={index} className="mb-5 flex justify-between items-center">
                  {user.username}
                  <button onClick={()=>friendRequestHandler(user._id)} className="ml-2 bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-3 rounded">
                    Send Request
                  </button>
                </li>
              ))}
              ...
              <p className="text-white p-4 text-center flex justify-center items-center">Search again</p>
            )}
        </div>
